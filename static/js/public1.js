...
function set_bg(tmp_width,tmp_height,is_no_bg){
    ...
    if(is_no_bg!=1)setTimeout(set_bg2,500);
    if(!tmp_height){
        tmp_height=200;
    }
...
function set_bg2(){
    ...
    /*var bg_div = '<div id="bg_div" style= "position:absolute;left:0px;top:0px;filter:alpha(opacity=30);z-index:40;width:'+$(document).width()+'px;height:'+height+'px;background-color:#000000;opacity:0.3;" align="center"><iframe style="position:absolute; visibility:inherit;background-color:#000000; top:0px; left:0px; width:'+$(document).width()+'px; height:'+height+'px; z-index:-1;filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\';" frameborder="0" src="/bg_div.html"></iframe></div>';
    $('body').append(bg_div);*/
}

//加载中
function loading(load_id){
    $('#'+load_id).html('<div class="loading"><img src="/images/VNew/loading-wait-bg.gif"/></div>');
}
...
//新的信息提示
function show_msg_new(msg,width,height,time,top_height,title){
    if(!width)width=300;
...
function show_msg_new6(msg,width,height,time,top_height,title){
    if(!width)width=300;
    if(!height)height=150;
    //alert(2);
    var title=title?title:"信息提示";
    ...
    w6.denyResize();
    w6.setIcon();
    w6.attachHTMLString(msg);
    if(time){
        setTimeout('close_window2(6)',time);
    }
}
...
//创建新窗体
function get_url_window3(url,title,width,height){
	if(!width)width=670;
	if(!height)height=430;
	var tmp_arr = set_bg(width,height);
	var top_height = tmp_arr[0];
	var left_width = tmp_arr[1];
    var isWin = dhxWins.isWindow("w3");
	if(isWin==false){
		w2=dhxWins.createWindow("w3",left_width,top_height,width,height);
	}else{
		dhxWins.window("w3").setDimension(width,height);
		dhxWins.window("w3").setPosition(left_width,top_height);
	}
	dhxWins.window("w3").setText(title);
	dhxWins.window("w3").button("park").hide();
	dhxWins.window("w3").button("minmax1").hide();
	dhxWins.window("w3").denyResize();
	var t=url+'&etc='+new Date().getTime();
	dhxWins.window("w3").attachURL(url);
	dhxWins.window("w3").attachEvent("onClose", function(win){
       $('#bg_div').remove();
	   return true;
    });
}
...
function set_window_id_val(wid,obj_id,val){
    ...
    if(iframe_obj[0].contentWindow.addPopupObject){
        iframe_obj[0].contentWindow.addPopupObject.popup.find('#'+obj_id).val(val);
    }else if(iframe_obj.find('#'+obj_id)){
        iframe_obj.find('#'+obj_id).val(val);
    }
    if(iframe_obj[0].contentWindow.document.getElementById(obj_id)){
        var old_val = $.trim(iframe_obj[0].contentWindow.document.getElementById(obj_id).value);
        //old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1
        // if(old_val =='#' || old_val =='###' || old_val=='' || old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1 || old_val.indexOf('.com') != -1 || old_val.indexOf('.cn') != -1 || old_val.indexOf('.net') != -1){
        // }
        iframe_obj[0].contentWindow.document.getElementById(obj_id).value=val;
    }

    var iframe_obj=$('.dhtmlx_skin_dhx_skyblue div iframe');
    if(iframe_obj[0].contentWindow.document.getElementById(obj_id)){
        iframe_obj[0].contentWindow.document.getElementById(obj_id).value=val;
    }
}
...
//图标专用
function set_window_id_val_tubiao(wid,obj_id,val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
...
function show_msg_new2(msg,width,height,time){
    msg='<font style="font-size:14px; display:block; height:100%; background:#fff;">'+msg+'</font>';
    setTimeout(function(){
...
//是否删除
function exec_confirm(url,tag){
    if(tag){
        if(confirm("确定要删除会员等级和对应的会员卡吗？删除后将不可恢复！")){
            location.href=url;
        }
    }else{
        location.href=url;
...
function out_login(){
    ...
    if (typeof PMp == "undefined") {
        mp = parseInt(readCookie('ev_mp'));
    } else {
        mp = parseInt(PMp);
    }

    var is_hui_lian_ver = parseInt(readCookie('is_hui_lian_ver'));
    if (is_hui_lian_ver) {
        urlParam = '?is_huilian=1';
    } else if (mp === 1) {
        urlParam = '?mp='+ mp;
    }

    if(no_new && USERHOST){
        location.href='http://'+USERHOST+'/login.php'+urlParam;
    }else{
        location.href='/login.php'+urlParam;
    }
}
...
function out_login2(){
    ...
    writeCookie('ev_manager_id','',-1);
    writeCookie('portal_userid','',-1);
    location.href='http://www.evyun.cn';
}
...
function show_msg_txt(tmp_tag){
    ...
    if(!tag){
        $.post("/ajax_get_info.php", {type:"112",menu_sub:tmp_tag,u:time},
        function(data){
                if(data){
                    $('#v_msg_con').html(data);
                    $('#v_msg_txt').show();
...
function clear_pro_pic(id,pic_id){
    ...
    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"121",id:id,pic_id:pic_id,u:time},
    function(data){
            //alert(data);
            if(data){
...
//清除一元购图片
function clear_yyg_pic(id,pic_id){

    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"281",id:id,pic_id:pic_id,u:time},
    function(data){
            //alert(data);
            if(data){
                show_msg_new2('清除成功！',300,150);
...
function clear_doc_pic(id){
    ...
    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"122",id:id,u:time},
    function(data){
            //alert(data);
            if(data){
                show_msg_new2('清除成功！',300,150);
...
function clear_item_pic(id){
    ...
    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"124",id:id,u:time},
    function(data){
            //alert(data);
            if(data){
                show_msg_new2('清除成功！',300,150);
...
function clear_server_pic(id){
    ...
    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"126",id:id,u:time},
    function(data){
            //alert(data);
            if(data){
                show_msg_new2('清除成功！',300,150);
...
function insert_user_browse_path(title,path){
    var time =UTCTimeDemo();
    $.post("/ajax_get_info.php", {type:"123",title:title,url:path,u:time},
    function(data){});
}
      ...
      image.onload=function(){
        ...
        if(size>maxsize){
            if(maxsize<1024){
                ...
                alert("上传文件不能大于"+parseInt(maxsize/1024)+"M");
            }
            target.outerHTML=target.outerHTML;
            return false;
        }else{
...
//图标专用
function edit_window_module_param_val_new(obj_id, val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    $(iframe_obj[0].contentWindow.document.getElementById(obj_id)).html($.trim(val));
}
 ...
 * id 返回对象的ID
 * val 返回值
 * type 1 html赋值 0 文本框赋值
 * is_append 是否为追加内容
 */
function firstWindowParamVal(id,val,type,is_append){
    var iPicNum = winobj.mFrame[0].contentWindow.getPicNum();
    if (iPicNum >5) {
        alert('最多可以添加5张图片！');
        return false;
    }
    var iframe_obj=$(window.frames["navigate_iframe"].document);
    var obj = iframe_obj.find("#"+id);
    if (!type) {
        obj.val($.trim(val));
    } else {
        if (is_append) {
 ...
 * @returns {boolean}
 */
function check_price(number, decimals) {
    var is_number = false;
...
function set_window_pic_val(pic_url_id, show_pic_id, val, type){
    ...
    // iframe_obj[0].contentWindow.document.getElementById("edit_pic").style.display="block";
...
function set_window_id_val_two(wid,obj_id,val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    var old_val = $.trim(iframe_obj[1].contentWindow.document.getElementById(obj_id).value);
    //old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1
    // if(old_val =='#' || old_val =='###' || old_val=='' || old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1 || old_val.indexOf('.com') != -1 || old_val.indexOf('.cn') != -1 || old_val.indexOf('.net') != -1){
    // }
...
//给某个窗体传值
function set_window_id_val_one(obj_id,val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
...
//给某个窗体传值
function set_window_id_val_new(wid,obj_id,val,tag){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    var old_val = $.trim(iframe_obj[tag].contentWindow.document.getElementById(obj_id).value);
    //old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1
    // if(old_val =='#' || old_val =='###' || old_val=='' || old_val.indexOf('.php') != -1 || old_val.indexOf('.html') != -1 || old_val.indexOf('.com') != -1 || old_val.indexOf('.cn') != -1 || old_val.indexOf('.net') != -1){
    // }
    iframe_obj[tag].contentWindow.document.getElementById(obj_id).value=val;
}
...
function set_window_id_html_two(wid,obj_id,val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    iframe_obj[0].contentWindow.document.getElementById(obj_id).innerHTML=val;
}
function set_color_set_show(wid,obj_id,val){
    //访问父窗口
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    if(val){
        iframe_obj[0].contentWindow.document.getElementById(obj_id).style.display="none";
    }else{
        iframe_obj[0].contentWindow.document.getElementById(obj_id).style.display="inline-block";
    }
}
// 隐藏/显示某个窗口 div
...
function tj_get_window_location_class_title(module_id, con_id) {
    if (!module_id) return false;
    var iframe_obj=$('.dhtmlx_wins_body_inner div iframe');
    iframe_obj[0].contentWindow.location_class_name(module_id, con_id);
    window.parent.close_window2(2);
}
...
function sendParentIframeVal(isFirst,isSecond,paramID,paramVal,iframeLocation) {
    var iframeObj = '';
    if (isFirst == 1){
        iframeObj = $('#navigate_iframe');
        $(iframeObj[0].contentWindow.document.getElementById(paramID)).val(paramVal);
    } else if (isSecond == 1) {
        iframeObj = $('.dhtmlx_wins_body_inner div iframe');
        iframeObj[0].contentWindow.document.getElementById(paramID).value=paramVal;
    }
    window.parent.close_window2(iframeLocation);
}
 ...
 * 消息提示
 * @param hint // 提示文字
 * @param icon // 1：表示成功对钩，2：表示错误叉号，3：表示提示感叹号
 * @param closeTime // 自动关闭时间数值型(以秒计算) false或者非数字表示不自动关闭
 */
G_.msg = function (hint, icon, closeTime) {
    var tmpIcon = parseInt(icon) ? icon : 1,
        tmpCloseTime = parseInt(closeTime) ? closeTime : 1;
 ...
 * 小图标开启等待
 */
G_.waitMsgS = function () {
    G_.loadMsgV = $.evPopupPoint({
       icon     : 4,
       closeTime: false
   });
}
 ...
 * 小图标关闭等待
 */
G_.waitMsgE = function () {
    G_.loadMsgV.popupClose();
}
 ...
 * @returns {*|string}
 */
G_.replaceNewlineWithBr = function (text) {
    return text ? text.replace(/\r?\n|\r/g, '<br>') : '';
};

function removeSideClass(status) {
    if (parseInt(status) === 1) {
        $('.page-sidebar-empty').addClass('secondary-sidebar');
    } else {
        $('.page-sidebar-empty').removeClass('secondary-sidebar');
    }
